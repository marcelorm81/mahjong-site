...
export const BottomNav: React.FC<BottomNavProps> = ({ currentPage, onNavigate }) => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 h-[88px] short:h-14 bg-black/40 backdrop-blur-xl border-t border-white/10 shadow-[0_-10px_40px_rgba(0,0,0,0.5)] transition-all duration-300">
      <div 
        ref={navRef}
          ...
          {NAV_ITEMS.map((item) => {
            ...
            return (
              <motion.button
                ...
                className="relative flex flex-col items-center justify-center w-[72px] h-16 short:h-12 rounded-xl snap-center group"
                whileTap={{ scale: 0.9 }}
              >
                ...
                {/* Icon Image */}
                <div className={`
                  relative z-10 transition-transform duration-300 w-10 h-10 short:w-6 short:h-6
                  ${isActive ? 'scale-110 -translate-y-1 short:-translate-y-0.5' : 'group-hover:scale-105'}
                `}>
                  <img 
                    src={item.iconUrl} 
                    alt={item.label}
                ...
                {/* Label */}
                <span className={`
                  relative z-10 text-[9px] short:text-[8px] font-bold mt-1 short:mt-0.5 uppercase tracking-wider transition-colors duration-300
